...
DETAIL = [
    ...
    (5, "LINEログイン（LIFF）", "バックエンド・フロントエンド", 5, "W2–W3",
     ["LIFFアプリの本番設定（LIFF ID・エンドポイントURL・スコープ）と本番環境での動作確認",
      "LINE userId と会員の連携・新規登録導線（プロトタイプで土台を実装済み）",
      "既存メール認証との統合・アカウント紐付け、セッション発行・保護（同上）"],
    ...
    (11, "管理画面の本番化", "フロントエンド・バックエンド", 10, "W3–W6",
     ["会員・書類の確認／承認／停止／退会の実運用機能",
      ...
      "権限管理・操作の監査ログ"],
     "運営が日常業務を管理画面だけで回せる"),
    (12, "相性推薦 v1", "バックエンド", 4, "W5–W6",
     ["プロフィール項目に基づく相性表示（機械学習は行わない）",
      "文面のお客様確認の反映、「AI」表記の適正化（優良誤認の回避）",
      "※ AIアドバイザーの削除（2026年9月決定）に伴い、本工程の工数（4人日）は見直し予定（要確定）"],
     "相性表示が実用レベルで動作し、表記が適正"),
...
def build(path):
    ...
    st.append(Paragraph("2026年9月（改訂）　発行：ViFight", S["cover_meta"]))
    st.append(PageBreak())
    ...
    st.append(tbl(["ご対応いただくこと", "内容", "期日の目安"], [
        ...
        ["特典・表記の確認", "提携店ギフト券・紹介特典の条件確定、AI相性表示の文言確認。", "W5–W6"],
    ...
    st.append(tbl(["今回の納品に含む（v1.0）", "含まない（次フェーズでご提案）"], [
        ["LINEログイン（LIFF）／LINE・メール通知", "AIの本格学習（成婚データ駆動の推薦）"],
    ...
    st.append(tbl(["項目", "内容", "状態"], [
        ...
        ["工程12の工数", "AIアドバイザー削除（2026年9月決定）に伴う相性推薦 v1 の人日の見直し。", "要確定"],
    ], [CW * 0.22, CW * 0.62, CW * 0.16], soft_cols=()))

    # 14 短縮
    st.append(h1("14. 補足：1.5ヶ月での短縮納品について"))
    st.append(Paragraph("W6末（約1.5ヶ月）のβ（限定公開）時点で先行リリースする短縮案も可能です。その場合、実決済は審査承認後の開始、一部の作り込み（相性推薦 v1の精度向上・管理画面の一部運用機能・テスト範囲の拡充など）はリリース後の継続改善に回します。品質と安全性を優先する場合は、予備日を含む2ヶ月での一般公開・正式納品を推奨します。", S["body"]))
