...
- **AI アドバイザー（デモ）** … 仲人ノウハウをもとにした相談・アドバイス UI。本実装では RAG による応答生成を予定。
...
- **不動産シナジー訴求** … グループの不動産事業と連携した成婚特典（新居探しサポート・キャッシュバック）の訴求導線。
- **源泉徴収票の男性必須化** … 所得証明（源泉徴収票）を男性は提出必須・女性は任意に。年収欄の公開には所得証明の確認が必要。

---

## インフラ・運用方針

- **PWA（実装済み）** … `manifest` とアイコンを同梱済み。ホーム画面追加でアプリライクに利用可能。将来的にネイティブアプリ化（ストア配信）へ移行する。
- **GCP 構成** … Cloud Run（自動スケーリング）＋ Cloud SQL（PostgreSQL）＋ Cloud Storage を想定。アクセス増減に応じてインスタンスが自動伸縮する。
- **証明書類の取り扱い** … 身分証・独身証明・所得証明などの書類はプライベートバケットに保存し、閲覧は有効期限付きの署名付き URL 経由のみ。公開バケットには置かない。
- **定例保守** … 依存パッケージの定期更新、キャッシュのクリーンアップ、バグ修正を定例運用として実施。
